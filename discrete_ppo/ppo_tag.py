...
from models.cnn_agent import cnn_policy_net, cnn_value_net
from utils.memory import MainMemory, ProcessMemory
...
parser.add_argument("--batch-size", type=int, default=1000, help="batch_size, please keep it as a multiple of n_proc")
parser.add_argument("--full-ppo-iters", type=int, default=500, help="num times whole thing is run")
parser.add_argument("--seed", type=int, default=1, help="set random seed for reproducibility ")
parser.add_argument("--n-proc", type=int, default=1, help=" if running multi process data collection ")
...
def collect_and_merge_exp(envs, actors, memories, main_memory, max_iters):
    with concurrent.futures.ProcessPoolExecutor() as executor:
        result_memories = executor.map(
            collect_exp_single_actor, envs, actors, memories, [int(max_iters/len(envs))] * len(envs)
        )
    print("done collecting")

    for mem in result_memories:
        main_memory.merge_memories(mem)
        mem.clear_memory()
    print("done merging")
...
def create_env(env_name, seed, config):
...
if __name__ == "__main__":
    ...
    my_config = {"map": "goal_map.txt", "env_type": "goal", "reward_type": "distance"}
    ...
    # create multiple environments, actor and memory objects
    if args.n_proc > 1:
        envs = [create_env(args.env_name, i, my_config) for i in range(args.n_proc)]
        actors = [cnn_policy_net(n_channels, n_actions) for _ in range(args.n_proc)]
        memories = [ProcessMemory(id=i) for i in range(args.n_proc)]

    # create nn's
    main_actor = cnn_policy_net(n_channels, n_actions)
    critic = cnn_value_net(n_channels, n_actions)
    ...
    for iter in tqdm(range(args.full_ppo_iters + 1)):
        ...
        if args.n_proc > 1:
            collect_and_merge_exp(envs, actors, memories, main_memory, args.batch_size)
        else:
        ...
        if args.n_proc > 1:
            for actor in actors:
                actor.load_state_dict(main_actor.state_dict())
